...
const (
	ApiEndpointSubmit PostEndpoint = "/submit"
)
...
func NewPostRequest(server string, port int, endpoint PostEndpoint) *PostRequest {
	ep := fmt.Sprintf("%v:%v%v", server, port, string(endpoint))
